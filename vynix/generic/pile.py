...
from ._type_lock import TypeLock
from uuid import UUID

from .ids import to_list_type, canonical_id

E = TypeVar("E", bound=Element)


def _validate_progression(value, collections: dict):
    if not value:
    ...
    for i in value_set:
        if canonical_id(i) not in collections.keys():
            raise ValueError(
...
def _validate_type_lock(value: Any) -> TypeLock:
    if isinstance(value, TypeLock):
        return value
    if value is None:
        return TypeLock()
    if isinstance(value, dict):
        return TypeLock(**value)
    raise TypeError("Invalid type for type_lock")
...
class Pile(Element, Collective[E], Generic[E]):
    ...
    type_lock: TypeLock = Field(default_factory=TypeLock, frozen=True)
    collections: dict[UUID, E] = Field(default_factory=dict)
    progression: Progression = Field(default_factory=Progression)

    @model_validator(mode="before")
    def _validate_data(cls, data: dict):
        tl = _validate_type_lock(data.get("type_lock"))
        data["type_lock"] = tl
        data["collections"] = tl.validate_items(data.get("collections"))
        data["progression"] = _validate_progression(data.get("progression"), data["collections"])
        return data

    @field_serializer("type_lock")
    def _serialize_type_lock(self, v: TypeLock) -> dict[str, Any]:
        return v.to_dict()

    @field_serializer("collections")
    def _serialize_collection(self, v: dict[UUID, E]) -> list[dict[str, Any]]:
        return [v[i].to_dict() for i in self.progression]
    ...
    def include(self, item: Any, /, as_single: bool = True):
        if isinstance(item, Pile):
            if not as_single:
                item = list(item)
        try:
            item_dict = self.type_lock.validate_items(item)
        except Exception:
            return False

        to_update = {}
        for i in item_dict.keys():
            if i not in self.progression.order:
                to_update[i] = item_dict[i]

        self.collections.update(to_update)
        self.progression.order.extend(to_update.keys())
        return True
...
class Pile(Collective[E], Generic[E]):
    ...
    def __init__(
        ...
        super().__init__()
        self._type_lock = TypeLock(
            item_type=item_type, strict_type=strict_type
        )
        self._collections = _validate_collections(
            collections, item_type, strict_type
        )
...
def _validate_collections(value, type_lock: TypeLock):
    if not value:
        return {}
    ...
    for i in value:
        ...
        if type_lock.item_type:
            if type_lock.strict_type:
                if type(i) not in type_lock.item_type:
                    raise ValidationError.from_value(
                        i,
                        expected=f"One of {type_lock.item_type}, no subclasses allowed.",
                    )
            else:
                if not any(
                    issubclass(type(i), t) for t in type_lock.item_type
                ):
                    raise ValidationError.from_value(
                        i,
                        expected=f"One of {type_lock.item_type} or the subclasses",
                    )
        else:
