    ...
    **kwargs: Any,
    ...
    if not parser:
        if str_type == "xml" and not parser:
            from ._utils import LU

            parser = partial(LU.xml_to_dict)
...
def _to_dict(
    ...
    **kwargs: Any,
    ...
    if isinstance(input_, _SINGLETONE_TYPES):
...
class ToDictParams(Params):
    """
    Parameters for to_dict function.
    """

    _func: ClassVar = to_dict
    ...
    parser: Callable[[str], Any] | None = None
    recursive: bool = False
    max_recursive_depth: int | None = 5
    recursive_python_only: bool = True
    use_enum_values: bool = True
