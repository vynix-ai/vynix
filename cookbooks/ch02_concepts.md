...
That approach was **single-turn**: each call to `chat()` did **not** add messages to the conversation history. Now, we’ll explore **vynix’s** architecture and focus on **multi-turn** usage with `branch.communicate()`, which **does** store messages for continuous dialogue.

---

## 1. Architecture & Design Philosophy

vynix is deliberately **modular**. When building a “Customer Service” bot, these key pieces unite:
...
Here’s a **vynix** approach for multi-turn conversation. Let’s define a simple Tool, an iModel, and a Branch that uses `communicate()`.
...
vynix automatically log conversation events via a LogManager. You can configure log manager by passing a config at branch creation:
