...
- vynix delivers 25-45% faster cold-start performance than the next-best
...
- **Cold Composite Performance** (geomean of medians; excludes imports): vynix
  **233 ms** vs LangGraph **421 ms** → **LangGraph is 81% slower** (**vynix is
  45% faster**)
- **Memory Efficiency**: vynix 40.5MB RSS vs LangGraph 63.3MB (36% lower)
- **Consistency**: First-place performance in 4/4 cold-start categories
- **Orchestrator Setup**: vynix 299ms vs LangGraph 410ms (37% faster)
...
composite (excluding `data_processing`): vynix **322.5 ms** vs LangGraph
**417.2 ms** → **LangGraph is 29% slower** (**vynix is 23% faster**).
...
| -------------- | ----------- | -------- | --------- | -------- | ------- |
| **vynix**    | 299.4       | 347.1    | 284-412   | 45.7     | —       |
...
| Framework      | Median (ms) | P95 (ms) | Range     | RSS (MB) | vs Best |
| -------------- | ----------- | -------- | --------- | -------- | ------- |
| **vynix**    | 326.6       | 377.6    | 304-391   | 47.6     | —       |
...
| Framework      | Median (ms) | P95 (ms) | Range     | RSS (MB) | vs Best |
| -------------- | ----------- | -------- | --------- | -------- | ------- |
| **vynix**    | 343.0       | 421.5    | 307-576   | 48.4     | —       |
...
- **vynix**: 40.5 MB average RSS (36.0 MB USS) — most memory efficient
- **LangGraph**: 63.3 MB average RSS (**+56% vs vynix**)
...
- **Low variability**: See CSV for MAD/stdev; vynix shows tight ranges in
...
| -------------- | ---------------------- | --------- | -------- | ------- | ------------------------- |
| vynix        | Session()              | Yes       | None     | No      | Minimal runtime container |
...
| --------------------- | ----------- | ------------------------------------ |
| Serverless/Lambda     | vynix     | Fastest cold start (233ms composite) |
| Memory-Limited        | vynix     | Lowest footprint (40.5MB)            |
...
The benchmarks show vynix's performance characteristics in cold-start
scenarios:
...
_Last updated: September 2025 • vynix v0.17.7 • Python 3.10.15_ _Benchmark
