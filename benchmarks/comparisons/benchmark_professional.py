...
import subprocess, sys, statistics, textwrap, json, csv, psutil, os, re
    ...
    for run_number in range(RUNS):
        try:
            ...
            individual_runs.append({
                ...
                "wall_clock_duration_ms": round((end_time - start_time).total_seconds() * 1000, 3)
            })
...
def main():
    ...
    for category_name, category_data in benchmark_categories.items():
        ...
        print("=" * 110)
        print(f"{'Framework':<12} {'Median (ms)':<12} {'Min (ms)':<10} {'Max (ms)':<10} {'StdDev':<8} {'Range':<15} {'Memory Usage':<25}")
        print("-" * 110)
        ...
        for result, individual_runs in category_results:
            ...
            print(f"{framework:<12} {median:<12.1f} {min_val:<10.1f} {max_val:<10.1f} {stddev:<8.1f} {range_val:<15} {memory_info}")
    ...
    with open(summary_csv, 'w', newline='') as csvfile:
        fieldnames = ['category', 'category_description', 'framework', 'mean_ms', 'median_ms',
                     'stdev_ms', 'min_ms', 'max_ms', 'mean_memory_mb', 'max_memory_mb',
    ...
    with open(detailed_csv, 'w', newline='') as csvfile:
        fieldnames = ['category', 'category_description', 'framework', 'run_number',
                     'execution_time_ms', 'memory_before_mb', 'memory_after_mb', 'memory_delta_mb',
                     'wall_clock_start', 'wall_clock_end', 'wall_clock_duration_ms', 'error']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
