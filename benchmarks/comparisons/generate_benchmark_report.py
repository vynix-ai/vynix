...
def composite_section(out, by_cat_mode, title, categories, modes, lion_name):
    ...
    if lion_g is not None and second_g is not None:
        ...
        out.append(f"> **vynix composite advantage:** {adv:+.1f}% vs #2.")
        out.append("")
...
def main():
    ...
    ap.add_argument("--lion", default="lionagi", help="Canonical name for vynix rows")
    args = ap.parse_args()
    ...
    for cat, mode in sorted(by_cat_mode.keys()):
        ...
        # vynix vs #2 callout if Lion present
        if lion_pair and not math.isinf(best_val):
            ...
            if lion_val is not None and second_val is not None:
                ...
                if adv > -1e-9:  # #2 >= lion
                    out.append(f"> **vynix advantage:** median {adv:+.1f}% vs #2 in this table.")
                    out.append("")
