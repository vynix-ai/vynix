...
class CapabilityMonotonicity:
    ...
    def post(self, br: Branch, node: OpNode, result: Dict[str, Any]) -> bool:
        ...
        return (
            post_caps.issubset(self._pre_caps) or post_caps == self._pre_caps
        )
...
class LenientIPU:
    ...
    async def before_node(self, br: Branch, node: OpNode) -> None:
        for inv in self.invariants:
            ...
            if not ok:
                ...
                print(
                    f"[WARN][{inv.name}] pre-phase violation at node {node.id}"
                )
    ...
    async def after_node(
        ...
        for inv in self.invariants:
            ...
            if not ok:
                print(
                    f"[WARN][{inv.name}] post-phase violation at node {node.id}"
                )
