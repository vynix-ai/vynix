...
from lion2.errors import ItemNotFoundError
...
class Progression(Identifiable, Generic[T]):
    ...
    def _validate_ordering(cls, v: Any) -> list[UUID]:
        return validate_order(v)

    @field_serializer("order")
    def _serialize_order(self, v: list[UUID]) -> list[str]:
    ...
    def remove(self, item: Any, /) -> None:
        ...
        if missing:
            raise ItemNotFoundError("Items not found", items=missing)
    ...
    def index(self, item: Any, start: int = 0, end: int | None = None) -> int:
        ...
        if end is not None:
            return self.order.index(_id, start, end)
        return self.order.index(_id, start)
    ...
    def exclude(self, item: Any, /) -> bool:
        ...
        if item is None:
            return (
                False  # Explicitly handle None as invalid input for exclusion
            )
