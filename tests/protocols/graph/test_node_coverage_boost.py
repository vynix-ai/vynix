...
class TestNodeAdaptation:
    ...
    async def test_adapt_to_async_postgres(self):
        ...
        with patch(
            ...
            with patch.object(
                Node.__bases__[-2], "adapt_to_async"
            ) as mock_adapt:
                mock_adapt.return_value = {"adapted": "data"}
    ...
    async def test_adapt_from_async_postgres(self):
        ...
        with patch(
            ...
            with patch.object(
                Node.__bases__[-2], "adapt_from_async"
            ) as mock_adapt:
                mock_node = Node()
                mock_adapt.return_value = mock_node

                result = await Node.adapt_from_async(
                    test_data, "lionagi_async_pg"
                )
...
class TestPostgresAdapterIntegration:
    ...
    def test_ensure_postgres_adapter_available(self):
        ...
        with patch(
            ...
            with patch(
                "lionagi.adapters.async_postgres_adapter.vynixAsyncPostgresAdapter"
            ) as mock_adapter:
                with patch.object(
                    Node, "register_async_adapter"
                ) as mock_register:
                    _ensure_postgres_adapter()
