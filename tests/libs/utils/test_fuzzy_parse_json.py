...
class TestFuzzyJsonParsing:
    ...
    def test_unmatched_brackets_fixing(self):
        ...
        assert fuzzy_json('{"a": 1') == {
            "a": 1
        }, "Missing closing brace must be fixed"
...
```"""
        ...
        assert result == [
            {"a": 1},
            {"b": 2},
        ], "Multiple blocks must return list"
...
```"""
    ...
    def test_no_json_found(self):
        ...
        ), "No JSON should return empty list"
        assert (
            extract_json("```python\nprint('hello')\n```") == []
...
class TestFixJsonString:
    ...
    def test_nested_missing_brackets(self):
        ...
        assert (
            fix_json_string('{"a": {"b": {"c": 1') == '{"a": {"b": {"c": 1}}}'
        )
    ...
    def test_string_preservation(self):
        ...
        result = fix_json_string(
            '{"text": "Use [brackets] and {braces} in text"}'
        )
