...
from tests.utils.mock_factory import vynixMockFactory
...
class TestFanOutIn:
    ...
    async def test_fan_out_gather_with_mocked_branches(self):
        ...
        branches = {
            "security": vynixMockFactory.create_mocked_branch(
                name="security",
                response="No vulnerabilities found.",
            ),
            "performance": vynixMockFactory.create_mocked_branch(
                name="performance",
                response="Latency is within acceptable limits.",
            ),
            "ux": vynixMockFactory.create_mocked_branch(
                name="ux",
    ...
    async def test_fan_out_aggregation(self):
        ...
        synthesizer = vynixMockFactory.create_mocked_branch(
            name="synthesizer",
...
class TestSequentialAnalysis:
    ...
    async def test_sequential_communicate_pipeline(self):
        ...
        parser = vynixMockFactory.create_mocked_branch(
            name="parser",
            response="Parsed: {name: Alice, age: 30}",
        )
        validator = vynixMockFactory.create_mocked_branch(
            name="validator",
            response="Valid: all fields present and correctly typed.",
        )
        formatter = vynixMockFactory.create_mocked_branch(
            name="formatter",
            response="# Alice\n- Age: 30",
        )
...
class TestTournamentValidation:
    ...
    async def test_tournament_solver_and_judge_flow(self):
        ...
        solvers = [
            vynixMockFactory.create_mocked_branch(
                name=f"solver_{i}",
                response=f"Solution {i}: x = {i + 1}",
            )
            for i in range(3)
        ]
        judge = vynixMockFactory.create_mocked_branch(
            name="judge",
...
class TestConditionalFlows:
    ...
    async def test_conditional_routing(self):
        ...
        classifier = vynixMockFactory.create_mocked_branch(
            name="classifier",
            response="category: technical",
        )
        technical = vynixMockFactory.create_mocked_branch(
            name="technical",
            response="Technical analysis: the code uses O(n) complexity.",
        )
        general = vynixMockFactory.create_mocked_branch(
            name="general",
