...
vynix's `iModel` provides a unified interface for working with different LLM providers. Configure once, swap providers without changing application code.
...
vynix loads API keys and defaults from environment variables:
