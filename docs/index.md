...
vynix gives you typed, observable control over LLM calls. Instead of dumping a prompt into a black-box chain, you build explicit workflows from `Branch` (a single conversation thread), `iModel` (any provider behind a uniform interface), and `Session` (multi-branch orchestration with graph-based execution). Every call is async, every response can be parsed into Pydantic models, and every tool invocation is logged.
...
vynix defaults to `openai` with `gpt-4.1-mini`. See [Installation](quickstart/installation.md) for all providers and optional extras.
