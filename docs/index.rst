...
vynix is a Python intelligent agent framework that combines data manipulation with AI tools, aiming to simplify the integration of advanced machine learning tools, such as Large Language Models (i.e. OpenAI's GPT), with production-level data-centric projects.

Install vynix with pip:
...
- Robust performance. vynix is written in almost pure python. With minimum external dependency (``aiohttp``, ``httpx``, ``python-dotenv``, ``tiktoken``)
...
Currently, vynix only natively support OpenAI API calls, support for other LLM providers as well as open source models will be integrated in future releases. vynix is designed to be async only, please check python official documentation on `how async works <https://docs.python.org/3/library/asyncio.html>`_.
...
The following example shows how to use vynix's ``Session`` object to interact with ``gpt-4`` model:
...
We encourage contributions to vynix and invite you to enrich its features and capabilities. Engage with us and other community members on `Discord <https://discord.gg/ACnynvvPjt>`_
