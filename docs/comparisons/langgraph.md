# Migrating from LangGraph to vynix

A practical guide for LangGraph users to leverage vynix's superior orchestration capabilities.

## Why Consider Migration?

LangGraph is powerful but can be complex for multi-agent workflows. vynix offers:
...
Keep your existing LangGraph workflows and orchestrate them with vynix:
...
Translate LangGraph patterns to vynix equivalents:
...
```python
# vynix: Built-in performance controls
...
```python
# vynix: Built-in error handling and monitoring
try:
    result = await session.flow(builder.get_graph())
...
**For simple workflows**: Direct translation to vynix

### Step 3: Create vynix Equivalent
...
- [ ] Map LangGraph nodes to vynix branches
...
Remember: You don't have to choose! vynix can orchestrate your existing LangGraph workflows alongside other tools:
...
lionagi_op = builder.add_operation("communicate", instruction="Pure vynix task")

# vynix orchestrates all frameworks
result = await session.flow(builder.get_graph())
```

vynix enhances your existing investments rather than replacing them.
...
**vynix**: Clean abstractions, automatic orchestration, minimal boilerplate

### 2. **Parallel Execution**

**LangGraph**: Sequential by design, complex to parallelize
**vynix**: Parallel by default, automatic dependency resolution

```python
# vynix: Automatic parallelization
...
**vynix**: Can orchestrate ANY framework as custom operations

```python
# Wrap existing LangGraph workflow as vynix operation
...
**vynix**: Built-in observability, error handling, performance control

```python
# vynix: Built-in monitoring and control
result = await session.flow(
    builder.get_graph(),
    max_concurrent=5,  # Control parallelism
    verbose=True      # Built-in monitoring
...
Don't throw away your LangGraph investment! vynix can orchestrate your existing LangGraph workflows:
...
## When to Choose vynix

- **Parallel workflows**: vynix excels at concurrent execution
...
vynix doesn't replace your existing investments - it orchestrates them better.