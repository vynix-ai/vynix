...
vynix supports 12 providers through a unified `iModel` interface. All providers
work with the same `Branch` API -- swap the model and your application code stays
the same.

## How Provider Selection Works

Every LLM call in vynix goes through `iModel`, which wraps an `Endpoint` for a
...
If the requested model is not available locally, vynix will pull it from the
...
vynix integrates three CLI-based coding agents as first-class iModel providers.
...
vynix detects availability at import time via `shutil.which()`.
...
CLI providers have two layers of context:

- **Within a session**: the CLI agent manages its own context via
  `--resume`. lionagi stores the `session_id` automatically and passes
  it on subsequent calls.
- **Across sessions**: when a session grows too long, start a fresh CLI
  instance and use `progression=` to select which prior messages to
  inject as context. Branch's MessageManager is the durable record.
...
vynix. When `match_endpoint` does not recognize the provider name, it falls
...
**Rate limit errors (429)** -- vynix retries with exponential backoff
