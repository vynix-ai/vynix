...
vynix automatically loads from `.env` - no manual configuration needed.
...
async def test():
    ...
    reply = await branch.chat("Hello from vynix!")
    print(f"vynix says: {reply}")
...
vynix comes pre-configured for these providers:
