...
vynix loads API keys from environment variables or `.env` files automatically (via `pydantic-settings` and `python-dotenv`). Set at least one provider key.
...
vynix checks `.env`, `.env.local`, and `.secrets.env` in that order.
...
vynix ships with optional dependencies for specific features:
...
When you create a `Branch()` without specifying a model, vynix uses:
