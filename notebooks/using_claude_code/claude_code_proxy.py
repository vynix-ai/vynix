...
def check_claude_code():
    ...
    try:
        result = subprocess.run(
            [CLAUDE_CODE_BINARY, "--version"], capture_output=True, text=True
        )
        if result.returncode != 0:
            raise RuntimeError(
                f"Claude Code not found at {CLAUDE_CODE_BINARY}"
            )
...
async def startup_event():
    check_claude_code()
    logger.info(
        "Claude Code Proxy API (OpenAI Compatible) started successfully"
    )
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    try:
        ...
        cmd = [
            ...
            full_prompt,
            "--output-format",
            "stream-json" if request.stream else "json",
            "--max-turns",
            "1",
        ]
        ...
        try:
            ...
            return format_text_response(
                stdout.decode(), request_id, request.model
            )
...
def format_text_response(
    text: str, request_id: str, model: str
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Claude Code Proxy API Server - OpenAI Compatible"
    )
