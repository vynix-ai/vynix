...
vynix system. The Component class represents distinguishable, temporal
entities within the system, encapsulating both data and behavior pertinent to
individual elements processed or managed by vynix.
...
class Component(BaseModel):
    """
    Represents a distinguishable, temporal entity in the vynix system.
    ...
    last_updated: str = Field(
        default_factory=lambda: SysUtil.get_timestamp(sep=None)[:-6],
        title="Last Updated",
        description="The UTC timestamp of the last update.",
        validation_alias=AliasChoices("updated_on", "update_date"),
        alias="updated",
    )
    ...
    def __setattr__(self, name, value):
        ...
        if name in self.model_fields or name in self.extra_fields:
            self.last_updated = SysUtil.get_timestamp(sep=None)[:-6]
    ...
    def __str__(self):
        return f"{self.class_name()}(id: {self.ln_id}, created: \
            {self.timestamp[:-6]}, updated: {self.last_updated[:-6]})"
