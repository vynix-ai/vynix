...
from lionagi.ln._types import Undefined, Unset
...
class TestToListCoercion:
    ...
    def test_handling_scalars_and_strings(self):
        ...
        assert to_list("abc") == ["abc"], "to_list('abc') must return ['abc'] not ['a', 'b', 'c']"
    ...
    def test_handling_mappings_and_structs(self):
        ...
        assert to_list(struct_input) == [struct_input], "msgspec.Struct must return [struct]"
        ...
        assert to_list(model_input) == [model_input], "Pydantic model must return [model]"

    def test_handling_iterables(self):
        """Test handling of various iterable types."""
        # List
        assert to_list([1, 2, 3]) == [1, 2, 3], "List input should return same list"

        # Tuple
        assert to_list((1, 2, 3)) == [1, 2, 3], "Tuple input should return list"
...
class TestToListTransformations:
    ...
    def test_flatten_tuple_set_control(self):
        ...
        # Test with sets
        input_with_set = [{1, 2}]
        result1 = to_list(input_with_set, flatten=True, flatten_tuple_set=False)
        assert result1 == [{1, 2}], "Sets not flattened when flatten_tuple_set=False"

        result2 = to_list(input_with_set, flatten=True, flatten_tuple_set=True)
        assert set(result2) == {1, 2}, "Sets flattened when flatten_tuple_set=True"

        # Test with frozensets
        input_with_frozenset = [frozenset([3, 4])]
        result3 = to_list(input_with_frozenset, flatten=True, flatten_tuple_set=False)
        assert result3 == [
            frozenset([3, 4])
...
class TestToListUniqueness:
    ...
    def test_unique_unhashable_types(self):
        ...
        assert len(result) == 3, "Unique must remove duplicates of unhashable types"
        assert result[0] == {"a": 1}
        assert result[1] == {"b": 2}
    ...
    def test_unique_unhashable_order_insensitivity(self):
        ...
        assert result[0] == {"k1": 1, "k2": 2}, "First occurrence must be preserved"
        ...
        assert len(result2) == 1, "Nested dicts with same content must be considered duplicates"
    ...
    def test_complex_transformation_combinations(self):
        ...
        assert len(result) == 5, f"Expected 5 unique elements, got {len(result)}: {result}"
