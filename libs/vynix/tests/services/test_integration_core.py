...
from lionagi.services.middleware import (
    CallMW, PolicyGateMW, MetricsMW, RedactionMW, HookedMiddleware,
    RetryMW, CircuitBreakerMW
)
...
class TestIntegrationCore:
    ...
    @pytest.mark.anyio
    async def test_full_pipeline_via_imodel_p1_requirement(self, test_service, hook_registry, caplog):
    ...
    @pytest.mark.anyio
    async def test_middleware_execution_order_policy_metrics_hooks_service(self, test_service, hook_registry):
        ...
        with patch.object(PolicyGateMW, '__call__', track_policy), \
    ...
    @pytest.mark.anyio
    async def test_streaming_integration_with_hooks_and_middleware(self, test_service, hook_registry):
    ...
    @pytest.mark.anyio
    async def test_observability_correlation_call_id_branch_id(self, test_service, hook_registry, caplog):
