...
def capability_set(draw, min_size=0, max_size=10):
    ...
    return set(
        draw(st.lists(capability_string(), min_size=min_size, max_size=max_size, unique=True))
    )
...
def wildcard_capability_set(draw, min_size=0, max_size=5):
    ...
    num_wildcards = draw(
        st.integers(min_value=0, max_value=min(len(wildcard_prefixes), max_size // 2))
    )
    wildcards = set(
        draw(
            st.lists(
                st.sampled_from(wildcard_prefixes),
                min_size=0,
...
class TestCapabilityModelCore:
    ...
    def test_call_context_creation_with_capabilities(self):
        ...
        ctx = CallContext.new(branch_id=uuid4(), capabilities=caps)
    ...
    def test_service_declared_vs_request_declared_precedence(self):
        ...
        ctx = CallContext.new(
            branch_id=uuid4(), capabilities=set(), service_requires=service_requires
        )
...
class TestPropertyBasedCapabilityAlgebra:
    ...
    def test_capability_coverage_reflexivity(self, available, required):
        ...
        if required.issubset(available):
            assert policy_gate._check_capabilities(
                available, required
    ...
    def test_capability_coverage_with_disjoint_sets(self, available, required):
        ...
        assert not policy_gate._check_capabilities(
            available, required
        ), f"Disjoint sets should not match: available={available}, required={required}"

    @given(
        wildcard_caps=wildcard_capability_set(min_size=1),
        specific_caps=capability_set(min_size=1),
    )
    ...
    def test_wildcard_coverage_property(self, wildcard_caps, specific_caps):
        ...
        for wildcard in wildcard_caps:
            if wildcard.endswith("*"):
                ...
                assert policy_gate._capability_covers(
                    {wildcard}, matching_specific
...
class TestRealisticAttackScenarios:
    ...
    def test_malicious_capability_pattern_prevention(self, legitimate_caps, malicious_patterns):
        ...
        assume(
            not any(cap.endswith("*") or "admin" in cap or "root" in cap for cap in legitimate_caps)
        )
