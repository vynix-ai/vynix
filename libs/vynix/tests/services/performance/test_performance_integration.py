...
from lionagi.services.endpoint import RequestModel, ChatRequestModel
...
class MockFullPipelineService:
    ...
    async def stream(self, req: RequestModel, *, ctx: CallContext) -> AsyncIterator[dict]:
        ...
        for i in range(chunk_count):
            if self.failure_rate > 0 and i > 10 and (self.call_count * self.failure_rate) > self.failure_count:
                self.failure_count += 1
...
def integration_request():
    ...
    return ChatRequestModel(
        messages=[
            ...
            {"role": "user", "content": "Write a detailed explanation of quantum computing principles and applications in modern technology."}
        ],
        model="gpt-4",
        temperature=0.7,
        max_tokens=2048
...
class TestFullPipelineLatency:
    ...
    async def test_baseline_pipeline_latency(self, integration_request):
        ...
        assert pipeline_latency < expected_latency, f"Pipeline overhead too high: {(pipeline_latency - 0.05) * 1000:.1f}ms"
    ...
    async def test_pipeline_with_middleware_stack(self, integration_request):
        ...
        assert result is not None
        assert middleware_latency < 0.5, f"Middleware latency too high: {middleware_latency * 1000:.1f}ms"
        ...
        assert middleware_latency < (service_time + expected_overhead + 0.05), \
    ...
    async def test_pipeline_streaming_latency(self, integration_request):
        ...
        service = MockFullPipelineService(
            latency_ms=80,
...
class TestConcurrentRequestHandling:
    ...
    async def test_concurrent_calls_scalability(self, integration_request):
        ...
        assert throughput_10 > throughput_1 * 3, f"Poor scaling: {throughput_10:.1f} vs {throughput_1:.1f}"
        assert throughput_20 > throughput_1 * 5, f"Poor scaling at 20: {throughput_20:.1f} vs {throughput_1:.1f}"
        
        # Higher concurrency should not degrade drastically
        if 40 in results:
            throughput_40 = results[40]["throughput"]
            assert throughput_40 > throughput_20 * 0.7, "Performance degradation at high concurrency"
    ...
    async def test_mixed_workload_handling(self, integration_request):
        ...
        assert total_stream_chunks == expected_stream_chunks, f"Missing stream chunks: {total_stream_chunks}/{expected_stream_chunks}"
    ...
    async def test_burst_traffic_handling(self, integration_request):
        ...
        for burst in range(3):  # 3 bursts
            ...
            print(f"    Burst {burst + 1} completed: {len(burst_results)} calls in {burst_time:.2f}s")
...
class TestResourceUtilization:
    ...
    async def test_memory_utilization_under_load(self, integration_request):
        ...
        for wave in range(5):  # 5 waves of requests
            ...
            print(f"  Wave {wave + 1}: {len(wave_results)} completed, memory delta: {memory_samples[-1]:.1f}MB")
        ...
        assert trace_memory_mb < 20, f"Tracemalloc detected high memory usage: {trace_memory_mb:.1f}MB"
    ...
    async def test_resource_cleanup_after_load(self, integration_request):
        ...
        assert stats["calls_completed"] == 150, f"Incorrect completion count: {stats['calls_completed']}"