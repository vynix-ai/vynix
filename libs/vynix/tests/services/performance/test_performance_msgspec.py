...
from lionagi.services.executor import ServiceCall, CallStatus, ExecutorConfig
...
class TestBatchSerialization:
    ...
    def test_batch_call_context_pydantic(self, benchmark):
        ...
        contexts = []
        for i in range(100):
            contexts.append(PydanticCallContext(
                call_id=uuid4(),
                branch_id=uuid4(), 
                deadline_s=time.time() + 300.0,
                capabilities={f"net.out:api.provider{i % 5}.com"},
...
class TestMemoryUsage:
    ...
    def test_memory_usage_msgspec_vs_pydantic(self):
        ...
        msgspec_size = sys.getsizeof(msgspec_contexts) + sum(sys.getsizeof(ctx) for ctx in msgspec_contexts)
        pydantic_size = sys.getsizeof(pydantic_contexts) + sum(sys.getsizeof(ctx) for ctx in pydantic_contexts)
        ...
        assert memory_improvement > 0.2, f"Memory improvement was only {memory_improvement:.1%}, expected > 20%"
