...
from lionagi.services.executor import ExecutorConfig, RateLimitedExecutor, ServiceCall, CallStatus
...
class SlowService(Service):
    ...
    async def call(self, req: DummyRequest, *, ctx: CallContext) -> dict[str, Any]:
        ...
        try:
            ...
            return {"result": f"completed after {self.delay_s}s", "call_count": self.call_count}
...
async def test_structured_startup_and_shutdown():
    ...
    total_terminal = final_stats["calls_completed"] + final_stats["calls_failed"] + final_stats["calls_cancelled"]
    assert total_terminal == 3, f"Expected 3 terminal calls, got {total_terminal}"
    assert final_stats["active_calls"] == 0, f"Found {final_stats['active_calls']} active calls after shutdown"
    
    # Shutdown should take some time (waiting for tasks) but not too long
    assert 0.1 < stop_duration < 5.0, f"Shutdown took {stop_duration:.2f}s - may indicate improper task management"
...
async def test_structured_shutdown_under_load():
    ...
    try:
        ...
        assert final_stats["calls_cancelled"] > 0, "Expected some calls to be cancelled during shutdown"
...
async def test_cancellation_propagation():
    ...
    try:
        ...
        stats = executor.stats
        assert stats["active_calls"] == 0, f"Expected 0 active calls after cancellation, got {stats['active_calls']}"
