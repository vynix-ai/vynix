...
class TestProactiveDeadlineEnforcement:
    ...
    async def test_proactive_deadline_enforcement_stream(
        ...
        with pytest.raises(anyio.get_cancelled_exc_class()):
            ...
            async for _ in service_with_slow_client.stream(basic_request, ctx=short_deadline_context):
                pass
    ...
    @pytest.mark.anyio
    async def test_no_deadline_allows_completion(
        self, service_with_slow_client, basic_request
    ):
...
class TestContextPropagationToSDK:
    ...
    def service_with_timeout_capture(self, mock_client_with_timeout_capture):
        ...
        return OpenAICompatibleService(
            client=mock_client_with_timeout_capture,
            name="timeout_test"
        )
...
class TestStreamYieldsImmediately:
    ...
    async def test_stream_yields_immediately(self, streaming_service):
        ...
        assert chunk_times[0] < 0.005, f"First chunk took {chunk_times[0]*1000:.1f}ms - too slow, might be buffering"
        
        # Verify chunks arrived with expected spacing
        if len(chunk_times) > 1:
            inter_chunk_delay = chunk_times[1] - chunk_times[0]
            assert 0.008 <= inter_chunk_delay <= 0.020, f"Inter-chunk delay: {inter_chunk_delay*1000:.1f}ms"
...
class TestServiceCapabilityDeclaration:
    ...
    def test_factory_function_capabilities(self):
        ...
        from lionagi.services.openai import create_openai_service, create_anthropic_service
