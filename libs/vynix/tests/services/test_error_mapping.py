...
class TestV1TransportHTTPXErrorMapping:
    ...
    async def test_http_status_retryable_errors(self):
        ...
        retryable_statuses = [
            (429, "Rate limited"),           # Rate limit - special case, should be RateLimitError
        ...
        for status_code, reason in retryable_statuses:
            ...
            mock_response_data = {"error": f"Test {status_code} error", "code": status_code}
            ...
            if status_code == 429:
                ...
                assert isinstance(error, RateLimitError), f"429 should map to RateLimitError, got {type(error)}"
                assert error.retry_after == 60.0, f"Expected retry_after=60.0, got {error.retry_after}"
                ...
                error = exc_info.value
                assert isinstance(error, RetryableError), f"{status_code} should map to RetryableError, got {type(error)}"
                assert error.retryable is True, f"{status_code} error should be retryable"
                assert str(status_code) in str(error), f"Error message should contain status code {status_code}"
    ...
    async def test_http_status_non_retryable_errors(self):
        ...
        for status_code, reason in non_retryable_statuses:
            ...
            error = exc_info.value
            assert isinstance(error, NonRetryableError), f"{status_code} should map to NonRetryableError, got {type(error)}"
    ...
    async def test_httpx_exception_mapping(self):
        ...
        for httpx_exception, expected_error_type, expected_message_part in test_cases:
            ...
            error = exc_info.value
            assert expected_message_part in str(error).lower(), (
    ...
    async def test_rate_limit_retry_after_parsing(self):
        ...
        for retry_after_str, expected_seconds in retry_after_cases:
            ...
            error = exc_info.value
            assert error.retry_after == expected_seconds, (
    ...
    async def test_rate_limit_missing_retry_after(self):
        ...
        assert error.retry_after == 60.0, f"Expected default retry_after=60.0, got {error.retry_after}"
    ...
    async def test_error_context_information(self):
        ...
        assert len(response_preview) <= 500 + len("... [truncated]"), "Response preview should be truncated"
    ...
    async def test_response_truncation_behavior(self):
        ...
        assert len(response_preview) <= 520, f"Response preview too long: {len(response_preview)} chars"
        assert response_preview.endswith("... [truncated]"), "Large response should be truncated with marker"
