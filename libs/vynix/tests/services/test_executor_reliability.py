...
async def test_executor_queue_wait_deadline_critical_flaw():
    ...
    try:
        ...
        assert (
            elapsed < 2.0
        ), f"Call took {elapsed:.2f}s but should have failed after ~1s due to deadline"
    ...
    try:
        ...
        async with anyio.create_task_group() as tg:
            for i in range(25):
                ...
                call = await executor.submit_call(
                    service, DummyRequest(content=f"request_{i}"), ctx
                )
                calls.append(call)
        ...
        assert (
            elapsed >= 2.0
        ), f"25 calls completed too quickly ({elapsed:.2f}s), rate limiting may not be working"
...
async def test_efficient_queuing_no_polling():
    ...
    try:
        ...
        assert (
            latency < 0.05
        ), f"Pickup latency {latency * 1000:.1f}ms too high - suggests polling behavior"
...
async def test_concurrency_safety_stress():
    ...
    try:
        ...
        assert (
            stats["calls_queued"] == expected_queued
        ), f"Expected {expected_queued} queued, got {stats['calls_queued']}"
        assert (
            actual_total == expected_queued
        ), f"Total processed ({actual_total}) != queued ({expected_queued})"
        assert (
            stats["active_calls"] == 0
        ), f"Should have 0 active calls, got {stats['active_calls']}"
...
async def test_executor_streaming_concurrency_limits():
    ...
    try:
        ...
        async def collect_stream(stream_id: int):
            ctx = CallContext.with_timeout(uuid4(), timeout_s=10.0)
            chunks = []
            async for chunk in executor.submit_stream(
                service, DummyRequest(content=f"stream_{stream_id}"), ctx
            ):
                chunks.append(chunk)
            return chunks
        ...
        assert (
            elapsed >= 0.5
        ), f"Streams completed too quickly ({elapsed:.2f}s), concurrency limit may not be working"
