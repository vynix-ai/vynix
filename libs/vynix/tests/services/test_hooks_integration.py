...
class TestV1HooksIntegration:
    ...
    async def test_hooked_middleware_integration_with_service_pipeline(self):
        ...
        assert result["response"] == "processed test-integration", "Service result should flow through"
        ...
        assert post_hook["result_keys"] == ["response", "call_id"], "Post-hook should have result data"
    ...
    async def test_hooked_middleware_error_handling_and_error_hooks(self):
        ...
        assert "Mock service failure" in error_capture["error_message"], "Should capture error message"
    ...
    async def test_hooked_middleware_streaming_integration_comprehensive(self):
        ...
        async def chunk_transform_hook(event: HookEvent, chunk: dict) -> dict:
            transformed = {**chunk, "transformed": True, "hook_applied": "middleware_test"}
            chunk_transformations.append(f"chunk_{chunk['chunk']}")
            return transformed
            
        # Post-stream hook
        async def post_stream_hook(event: HookEvent):
            execution_log.append(f"post_stream_completed_chunks_{event.metadata.get('chunk_count', 0)}")
        ...
        for i, chunk in enumerate(stream_results):
            ...
            assert chunk["hook_applied"] == "middleware_test", f"Chunk {i} should have hook metadata"
            
        # Verify hook execution order
        assert "pre_stream_started" in execution_log, "Pre-stream hook should execute"
        assert "post_stream_completed_chunks_3" in execution_log, "Post-stream hook should execute with correct count"
        
        # Verify all chunks were transformed
        assert len(chunk_transformations) == 3, "All chunks should be transformed"
        assert chunk_transformations == ["chunk_0", "chunk_1", "chunk_2"], "Chunks should be processed in order"
    ...
    async def test_hooked_middleware_streaming_error_handling(self):
        ...
        assert "Mock streaming failure" in error_capture["error_message"], "Should capture error message"
    ...
    async def test_observability_event_emission_timing_and_content(self):
        ...
        assert post_call["service_name"] == "observable-service", "Post-call should have service name"
    ...
    async def test_hook_performance_impact_measurement(self):
        ...
        assert overhead_percent < 50.0, f"Hook overhead should be < 50% but was {overhead_percent:.1f}%"
        
        # Verify both services were called correctly
        assert service_no_hooks.call_count == runs_per_test, "No-hooks service should be called for each run"
        assert service_fast.call_count == runs_per_test, "With-hooks service should be called for each run"
    ...
    async def test_hooked_middleware_context_attribute_injection(self):
        ...
        assert capture["context_attrs"]["service_name"] == "explicit-service", "Context attrs should be preserved"
        assert capture["context_attrs"]["custom_attr"] == "test_value", "Custom attributes should be preserved"
    ...
    async def test_hooked_middleware_concurrent_execution_safety(self):
        ...
        assert execution_counts["count"] >= 8, "Should execute most hooks (allowing for race conditions)"
    ...
    async def test_hooked_middleware_service_name_fallback_behavior(self):
        ...
        assert service_name_captures[0] == "unknown", "Should use 'unknown' as fallback service name"
