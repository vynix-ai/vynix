...
class FastService(Service):
    ...
    async def call(self, req: DummyRequest, *, ctx: CallContext) -> dict[str, Any]:
        ...
        return {"result": "processed", "content": req.content, "tokens": getattr(req, 'estimated_tokens', 100)}
...
async def test_request_limit_enforcement():
    ...
    try:
        ...
        assert elapsed >= 1.8, f"Processing 7 requests with 3 req/sec limit took only {elapsed:.2f}s"
...
async def test_rate_limit_refresh_timing():
    ...
    try:
        ...
        assert 0.4 <= request_times[2] <= 0.8, f"Third request timing {request_times[2]:.2f}s unexpected"
        assert 0.4 <= request_times[3] <= 0.8, f"Fourth request timing {request_times[3]:.2f}s unexpected"
        
        # Requests 4-5 should be delayed by ~1.0s
        assert 0.9 <= request_times[4] <= 1.3, f"Fifth request timing {request_times[4]:.2f}s unexpected"
        assert 0.9 <= request_times[5] <= 1.3, f"Sixth request timing {request_times[5]:.2f}s unexpected"
...
async def test_rate_limit_accuracy_under_load():
    ...
    try:
        ...
        assert elapsed >= expected_time, f"Rate limiting failed: {num_requests} requests completed in {elapsed:.2f}s (expected >={expected_time:.1f}s)"
        
        # Most requests should complete
        assert completed >= num_requests * 0.8, f"Only {completed}/{num_requests} requests completed"
