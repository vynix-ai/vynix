...
from lionagi.services.middleware import PolicyGateMW, MetricsMW, RedactionMW
from lionagi.services.executor import RateLimitedExecutor, ExecutorConfig, ServiceCall
...
class TestEndToEndSecurityEnforcement:
    ...
    async def test_streaming_security_enforcement(self):
        ...
        with pytest.raises(PolicyError):
            async for chunk in policy_mw.stream(req, ctx_insufficient, 
        ...
        chunks = []
        async for chunk in policy_mw.stream(req, ctx_sufficient,
                                          lambda: mock_service.stream(req, ctx=ctx_sufficient)):
            chunks.append(chunk)
...
class TestSecurityAuditLogging:
    ...
    async def test_redaction_middleware_integration(self, caplog):
        ...
        log_records = [record.message for record in caplog.records if "Service call starting" in record.message]
    ...
    async def test_policy_metrics_redaction_middleware_stack(self, caplog):
        ...
        async def full_stack():
            return await redaction_mw(
                ...
                lambda: metrics_mw(
                    req, ctx_fail,
                    lambda: mock_service.call(req, ctx=ctx_fail)
                )
            )
        ...
        async def full_stack_success():
            return await redaction_mw(
                ...
                lambda: metrics_mw(
                    req, ctx_success,
                    lambda: mock_service.call(req, ctx=ctx_success)
                )
            )
    ...
    async def test_middleware_execution_order_security(self):
        ...
        with pytest.raises(PolicyError):
            await tracking_policy(
                req, ctx_fail,
                lambda: tracking_metrics(req, ctx_fail, lambda: mock_service.call(req, ctx=ctx_fail))
            )
        ...
        result = await tracking_policy(
            req, ctx_success,
            lambda: tracking_metrics(req, ctx_success, lambda: mock_service.call(req, ctx=ctx_success))
        )
        
        # Verify proper execution order
        expected_order = ["policy_start", "metrics_start", "metrics_success", "policy_success"]
