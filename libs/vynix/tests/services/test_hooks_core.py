...
class TestHooksCore:
    ...
    async def test_per_hook_soft_timeout_critical_flaw_validation(self):
        ...
        with patch("lionagi.services.hooks.logger") as mock_logger:
            ...
            assert emit_duration < 1.5, f"Emit should complete in ~1s with per-hook timeouts but took {emit_duration:.3f}s"
            assert emit_duration > 0.9, f"Emit should take close to timeout duration but took {emit_duration:.3f}s"
            ...
            assert "slow_completed" not in completion_times, "Slow hook should be cancelled by per-hook timeout"
    ...
    async def test_failure_isolation_robust_gather_critical(self):
        ...
        with patch("lionagi.services.hooks.logger") as mock_logger:
            ...
            assert "hook function failed" in error_call_args.lower() or "intentional" in error_call_args.lower()
    ...
    async def test_hook_registration_and_deregistration_lifecycle(self):
        ...
        assert registry.has_hooks(HookType.PRE_CALL), "Should have PRE_CALL hooks after registration"
    ...
    async def test_stream_hook_chaining_and_transformation(self):
        ...
        with patch("lionagi.services.hooks.logger") as mock_logger:
            ...
            assert "transform_4" not in transformation_log, "Fourth transform should NOT execute after failure"
            
            # Validate transformations applied before failure
            assert processed_chunk.get("metadata") == "added_by_hook_1", "First transformation should be applied"
            assert processed_chunk.get("transformed") is True, "Second transformation should be applied"
    ...
    async def test_hook_context_preservation_call_id_branch_id(self):
        ...
        assert captured["service_name"] == "context-test-service", "Service name should be preserved"
    ...
    async def test_error_hook_triggering_and_context_capture(self):
        ...
        stream_error_captured = next(e for e in error_events if e["hook_type"] == HookType.STREAM_ERROR)
        
        assert call_error_captured["error_data"] == "Test call error", "Call error data should be preserved"
        assert stream_error_captured["error_data"] == "Test stream error", "Stream error data should be preserved"
        assert stream_error_captured["context"]["chunk_index"] == 5, "Stream error context should include chunk info"
    ...
    async def test_global_hook_decorator_and_registry_integration(self):
        ...
        try:
            ...
            assert global_registry.has_hooks(HookType.PRE_CALL), "Global PRE_CALL hook should be registered"
            assert global_registry.has_hooks(HookType.STREAM_CHUNK), "Global STREAM_CHUNK hook should be registered"
            ...
            assert "global_stream_transform" in execution_log, "Global stream transform should execute"
            assert transformed_chunk.get("global_transform") is True, "Global transform should modify chunk"
