...
class MockTransportBuilder:
    ...
    @staticmethod
    def create_status_response(status_code: int, content: Dict[str, Any] = None, extra_headers: Dict[str, str] = None) -> MockTransport:
    ...
    @staticmethod
    def create_streaming_transport(chunks: List[Dict[str, Any]], delay: float = 0.01) -> MockTransport:
...
class ServiceTestBuilder:
    ...
    @staticmethod
    def create_mock_openai_service(mock_transport: MockTransport = None, service_name: str = "test_service") -> OpenAICompatibleService:
...
class TestErrorPropagationPipeline:
    ...
    @pytest.mark.anyio
    @pytest.mark.parametrize("status_code,error_type,retryable,validation", [
        (429, RateLimitError, True, lambda e: e.retry_after > 0),
        (500, RetryableError, True, lambda e: "Server error: 500" in e.message),
        (502, RetryableError, True, lambda e: "Server error: 502" in e.message),
        (503, RetryableError, True, lambda e: "Server error: 503" in e.message),
    ...
    async def test_http_status_to_service_error_propagation(self, status_code, error_type, retryable, validation):
...
class TestDeadlineEnforcementIntegration:
    ...
    async def test_streaming_deadline_enforcement(self):
        ...
        elapsed_time = time.time() - start_time
        assert elapsed_time < 0.2, f"Expected stream timeout ~100ms, got {elapsed_time*1000:.1f}ms"
...
class TestServiceCapabilityIntegration:
    ...
    async def test_service_capability_declaration_validation(self):
        ...
        with patch('lionagi.services.openai.AsyncOpenAI') as mock_openai:
            ...
            from lionagi.services.openai import create_anthropic_service, create_openrouter_service
...
class TestCompletePipelineLifecycle:
    ...
    async def test_service_middleware_integration_pipeline(self):
        ...
        async def mock_create(*args, **kwargs):
            mock_response = MagicMock()
            mock_response.model_dump.return_value = {"id": "middleware-test", "content": "response"}
            return mock_response
