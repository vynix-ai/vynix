...
from lionagi.services.resilience import CircuitBreakerConfig, CircuitBreakerMW, CircuitState
...
class ConfigurableService:
    ...
    def __init__(self, should_fail: bool = False, failure_type: Exception = None, 
                 chunk_count: int = 3, chunk_delay: float = 0.01, fail_after_chunks: int = None):
    ...
    async def stream_operation(self) -> AsyncIterator[dict[str, Any]]:
        ...
        for i in range(self.chunk_count):
            ...
            yield {"chunk": i + 1, "call": self.stream_call_count, "timestamp": time.time()}
...
class TestCircuitBreakerConcurrencySafety:
    ...
    async def test_concurrent_failure_counting_accuracy(self, circuit_config, mock_request):
        ...
        failing_service = ConfigurableService(should_fail=True, failure_type=RetryableError("Concurrent failure"))
    ...
    async def test_concurrent_mixed_success_failure_transitions(self, circuit_config, mock_request):
        ...
        circuit_mw = CircuitBreakerMW(circuit_config)
        failing_service = ConfigurableService(should_fail=True, failure_type=RetryableError("Failure"))
        ...
        async def mixed_concurrent_calls():
            ...
            async with anyio.create_task_group() as tg:
                ...
                for _ in range(10):
                    results.append(await tg.start_task_soon(
...
class TestCircuitBreakerStreamingPerformance:
    ...
    async def test_streaming_immediate_passthrough_no_buffering(self, circuit_mw, mock_request):
        ...
        for i, receive_time in enumerate(chunk_receive_times):
            ...
            assert abs(receive_time - expected_time) < 0.02, \
    ...
    @pytest.mark.anyio
    async def test_streaming_failure_propagation_and_state_management(self, circuit_mw, mock_request):
        ...
        failing_service = ConfigurableService(
            chunk_count=5, fail_after_chunks=2, 
        ...
        with pytest.raises(RetryableError, match="Stream failed after 2 chunks"):
            async for chunk in circuit_mw.stream(mock_request, ctx, failing_service.stream_operation):
...
class TestCircuitBreakerErrorTypeHandling:
    ...
    async def test_non_retryable_errors_do_not_affect_circuit(self, circuit_mw, mock_request):
        ...
        non_retryable_service = ConfigurableService(
            should_fail=True, 
            failure_type=NonRetryableError("Invalid request")
        )
    ...
    async def test_mixed_error_types_only_retryable_count(self, circuit_mw, mock_request):
        ...
        retryable_service = ConfigurableService(should_fail=True, failure_type=RetryableError("Network error"))
        non_retryable_service = ConfigurableService(should_fail=True, failure_type=NonRetryableError("Bad request"))
        ...
        with pytest.raises(NonRetryableError):
            await circuit_mw(mock_request, ctx, non_retryable_service.call_operation)  # Count: still 1
