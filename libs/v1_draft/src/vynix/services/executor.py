...
# Still need anyio for some functions not wrapped by lionagi
import anyio
...
class ServiceCall(msgspec.Struct, kw_only=True):
    ...
    _completion_event: Event | None = None
    ...
    async def wait_completion(self) -> Any:
        ...
        if not self._completion_event:
            raise _err.ServiceError("ServiceCall not properly initialized - no completion event")
...
class RateLimitedExecutor:
    ...
    def __init__(self, config: ExecutorConfig):
        ...
        from lionagi.ln import CapacityLimiter, Lock, Queue
        ...
        self.token_count = 0
        self.last_refresh = 0.0  # Will be set when async context starts
    ...
    async def submit_call(
        ...
        # Initialize the completion event in async context
        call._completion_event = Event()
    ...
    async def _run_processor(self) -> None:
        ...
        try:
            # Create a separate task group for executing calls
            async with anyio.create_task_group() as anyio_call_tg:
                call_task_group = TaskGroup(anyio_call_tg)
                while not self._shutdown_event.is_set():
                    try:
                        ...
                        # Process call using the separate call task group
                        self.active_calls[call.id] = call
                        # Start task within the call execution task group
                        call_task_group.start_soon(self._execute_call, call)

                        # Periodically cleanup completed
                        if len(self.completed_calls) > 100:
                            call_task_group.start_soon(self._cleanup_completed)
    ...
    async def __aenter__(self):
        ...
        # Initialize rate limiting timer now that we're in async context
        self.last_refresh = anyio.current_time()
        
        # Create task group and start background tasks
        # Create anyio task group and wrap in lionagi TaskGroup
        self._anyio_task_group = anyio.create_task_group()
        await self._anyio_task_group.__aenter__()
        self._task_group = TaskGroup(self._anyio_task_group)
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        # Cancel only pending/executing calls, not completed ones
        active_call_list = list(self.active_calls.values())
        for call in active_call_list:
            if call.status in (CallStatus.PENDING, CallStatus.EXECUTING, CallStatus.QUEUED):
                call.mark_cancelled()
        ...
        try:
            await self._anyio_task_group.__aexit__(exc_type, exc_val, exc_tb)
        except get_cancelled_exc_class():
