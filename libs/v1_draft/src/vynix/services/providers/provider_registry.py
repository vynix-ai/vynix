...
class ProviderRegistry:
    ...
    def resolve(
        ...
        Rules (following v0 approach):
          - If provider is explicit, use it (ignore any slashes in model name)
          - If provider is not explicit but model has 'prefix/model', extract provider from prefix
        ...
        """
        # Only parse provider from model if no explicit provider given
        if provider:
            effective_provider = provider
        else:
            pref_provider, _ = _parse_provider_prefix(model)
            effective_provider = pref_provider
...
def _parse_provider_prefix(model: str | None) -> tuple[str | None, str | None]:
    ...
    Following v0 approach: only extract provider from model if it makes sense
    as a provider prefix (first part before first slash).
    """
...
def register_builtin_adapters() -> None:
    ...
    from ..adapters.nvidia_nim_adapter import NvidiaNimAdapter
    from ..adapters.openai_adapter import OpenAIAdapter

    for a in (OpenAIAdapter(), GenericJSONAdapter(), ClaudeCodeAdapter(), NvidiaNimAdapter()):
